<?php
   ...
   if (!isset($_SESSION['sc_session']))
   {
       ...
       if (is_file($root . $_SESSION['scriptcase']['grid_departamento']['glo_nm_path_imag_temp'] . "/sc_apl_default_coprogreso.txt"))
       {
?>
...
<?php
   ...
   $Ord_Cmp = new grid_departamento_Ord_cmp(); 
   $Ord_Cmp->Ord_cmp_init();
   
class grid_departamento_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $use_alias, $tab_ger_campos, $tab_def_campos, $tab_def_seq, $tab_labels, $embbed, $tbar_pos, $_POST, $_GET;
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_departamento']['ordem_select_orig']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_departamento']['ordem_select_orig'] = array();
   }
   ...
   $tab_labels["clave"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_departamento']['labels']["clave"])) ? $_SESSION['sc_session'][$sc_init]['grid_departamento']['labels']["clave"] : "Clave";
   $tab_ger_campos['descripcion'] = "on";
   $tab_def_campos['descripcion'] = "descripcion";
   $tab_labels["descripcion"]   = (isset($_SESSION['sc_session'][$sc_init]['grid_departamento']['labels']["descripcion"])) ? $_SESSION['sc_session'][$sc_init]['grid_departamento']['labels']["descripcion"] : "Descripcion";
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['grid_departamento']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['grid_departamento']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['grid_departamento']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['grid_departamento']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['grid_departamento']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['grid_departamento']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['grid_departamento']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['grid_departamento']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       ...
       if (isset($tab_def_campos[$campo]))
       {
           ...
           if (isset($_SESSION['sc_session'][$sc_init]['grid_departamento']['ordem_quebra']))
           {
               foreach($_SESSION['sc_session'][$sc_init]['grid_departamento']['ordem_quebra'] as $campo_quebra => $resto) 
               {
                   foreach($resto as $sqldef => $ordem_ant) 
                   {
                       if ($sqldef == $tab_def_campos[$campo]) 
                       { 
                           $_SESSION['sc_session'][$sc_init]['grid_departamento']['ordem_quebra'][$campo][$sqldef] = $ordem;
           ...
           if (!$Ordem_tem_quebra)
           {
               $_SESSION['sc_session'][$sc_init]['grid_departamento']['ordem_select'][$tab_def_campos[$campo]] = $ordem;
           }   
       }
   }
?>
...
?>
  function scSubmitOrderCampos(sPos, sType) {
   ...
   return new Promise(function(resolve, reject) {$.ajax({
    type: "POST",
    url: "grid_departamento_order_campos.php",
    data: {
...
<?php
   ...
   $arr_order = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['grid_departamento']['ordem_select_orig'] : $_SESSION['sc_session'][$sc_init]['grid_departamento']['ordem_select'];
...
<?php
  if (isset($_SESSION['sc_session'][$sc_init]['grid_departamento']['ordem_select']) && $_SESSION['sc_session'][$sc_init]['grid_departamento']['ordem_select'] != $_SESSION['sc_session'][$sc_init]['grid_departamento']['ordem_select_orig']) {
      $disp_rest = "";
  }
   if (!$embbed)
   {
