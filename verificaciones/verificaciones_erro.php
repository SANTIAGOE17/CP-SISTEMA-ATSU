...
$old_error_handler = set_error_handler("verificaciones_scriptcase_error_handler");

function verificaciones_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['verificaciones']['contr_erro']) && $_SESSION['scriptcase']['verificaciones']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_verificaciones->contr_verificaciones->Apl_com_erro = true;
            if ($inicial_verificaciones->contr_verificaciones->NM_ajax_flag)
            {
                $inicial_verificaciones->contr_verificaciones->NM_ajax_info['errList']['geral_verificaciones'][] = $msg;
                return;
            }
            verificaciones_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function verificaciones_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['verificaciones'])  && '' != $_SESSION['scriptcase']['error_icon']['verificaciones'])  ? $_SESSION['scriptcase']['error_icon']['verificaciones']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['verificaciones']) && '' != $_SESSION['scriptcase']['error_close']['verificaciones']) ? $_SESSION['scriptcase']['error_close']['verificaciones'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['verificaciones']) && '' != $_SESSION['scriptcase']['error_span_title']['verificaciones']) ? $_SESSION['scriptcase']['error_span_title']['verificaciones'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['verificaciones']) && '' != $_SESSION['scriptcase']['error_icon_title']['verificaciones']) ? $_SESSION['scriptcase']['error_icon_title']['verificaciones'] : '';
?>
