...
$old_error_handler = set_error_handler("autorizaciones_scriptcase_error_handler");

function autorizaciones_scriptcase_error_handler($err_no, $err_msg, $err_file, $err_line, $exibe)
{
    ...
    if (isset($_SESSION['scriptcase']['autorizaciones']['contr_erro']) && $_SESSION['scriptcase']['autorizaciones']['contr_erro'] == 'on')
    {
    ...
    if (in_array($err_no, $errors_handled))
    {
        ...
        if ($bol_flag && $exibe)
        {
            ...
            $inicial_autorizaciones->contr_autorizaciones->Apl_com_erro = true;
            if ($inicial_autorizaciones->contr_autorizaciones->NM_ajax_flag)
            {
                $inicial_autorizaciones->contr_autorizaciones->NM_ajax_info['errList']['geral_autorizaciones'][] = $msg;
                return;
            }
            autorizaciones_scriptcase_error_display($msg, $err_no);
        }
        return;
    }
}

function autorizaciones_scriptcase_error_display($err_msg, $err_no)
{
    ...
    if (!$erro_ja_exibido)
    {
        ...
        $sErrorIcon      = (isset($_SESSION['scriptcase']['error_icon']['autorizaciones'])  && '' != $_SESSION['scriptcase']['error_icon']['autorizaciones'])  ? $_SESSION['scriptcase']['error_icon']['autorizaciones']  : "";
        $sCloseIcon      = (isset($_SESSION['scriptcase']['error_close']['autorizaciones']) && '' != $_SESSION['scriptcase']['error_close']['autorizaciones']) ? $_SESSION['scriptcase']['error_close']['autorizaciones'] : "<td><input class=\"scButton_default\" type=\"button\" onClick=\"document.getElementById('id_error_display_fixed').style.display = 'none'; document.getElementById('id_error_message_fixed').innerHTML = ''; return false\" value=\"X\" /></td>";

        $bSpanTitle      = (isset($_SESSION['scriptcase']['error_span_title']['autorizaciones']) && '' != $_SESSION['scriptcase']['error_span_title']['autorizaciones']) ? $_SESSION['scriptcase']['error_span_title']['autorizaciones'] : false;
        $sIconTitle      = (isset($_SESSION['scriptcase']['error_icon_title']['autorizaciones']) && '' != $_SESSION['scriptcase']['error_icon_title']['autorizaciones']) ? $_SESSION['scriptcase']['error_icon_title']['autorizaciones'] : '';
?>
