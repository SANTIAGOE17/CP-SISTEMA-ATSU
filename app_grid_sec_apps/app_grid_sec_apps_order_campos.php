<?php
   ...
   if (!isset($_SESSION['sc_session']))
   {
       ...
       if (is_file($root . $_SESSION['scriptcase']['app_grid_sec_apps']['glo_nm_path_imag_temp'] . "/sc_apl_default_coprogreso.txt"))
       {
?>
...
<?php
   ...
   $Ord_Cmp = new app_grid_sec_apps_Ord_cmp(); 
   $Ord_Cmp->Ord_cmp_init();
   
class app_grid_sec_apps_Ord_cmp
{
function Ord_cmp_init()
{
  global $sc_init, $path_img, $path_btn, $use_alias, $tab_ger_campos, $tab_def_campos, $tab_def_seq, $tab_labels, $embbed, $tbar_pos, $_POST, $_GET;
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['app_grid_sec_apps']['ordem_select_orig']))
   {
       $_SESSION['sc_session'][$sc_init]['app_grid_sec_apps']['ordem_select_orig'] = array();
   }
   ...
   $tab_def_campos['app_name'] = "app_name";
   $tab_labels["app_name"]   = (isset($_SESSION['sc_session'][$sc_init]['app_grid_sec_apps']['labels']["app_name"])) ? $_SESSION['sc_session'][$sc_init]['app_grid_sec_apps']['labels']["app_name"] : "" . $this->Nm_lang['lang_sec_app_name'] . "";
   $tab_ger_campos['description'] = "on";
   $tab_def_campos['description'] = "description";
   $tab_labels["description"]   = (isset($_SESSION['sc_session'][$sc_init]['app_grid_sec_apps']['labels']["description"])) ? $_SESSION['sc_session'][$sc_init]['app_grid_sec_apps']['labels']["description"] : "" . $this->Nm_lang['lang_sec_app_description'] . "";
   ...
   if (isset($_SESSION['sc_session'][$sc_init]['app_grid_sec_apps']['php_cmp_sel']) && !empty($_SESSION['sc_session'][$sc_init]['app_grid_sec_apps']['php_cmp_sel']))
   {
       foreach ($_SESSION['sc_session'][$sc_init]['app_grid_sec_apps']['php_cmp_sel'] as $NM_cada_field => $NM_cada_opc)
       {
           if ($NM_cada_opc == "off")
           {
              $tab_ger_campos[$NM_cada_field] = "none";
   ...
   if (!isset($_SESSION['sc_session'][$sc_init]['app_grid_sec_apps']['ordem_select']))
   {
       $_SESSION['sc_session'][$sc_init]['app_grid_sec_apps']['ordem_select'] = array();
   }
...
function Sel_processa_out_sel($campos_sel)
{
   ...
   $_SESSION['sc_session'][$sc_init]['app_grid_sec_apps']['ordem_select'] = array();
   $_SESSION['sc_session'][$sc_init]['app_grid_sec_apps']['ordem_grid']   = "";
   $_SESSION['sc_session'][$sc_init]['app_grid_sec_apps']['ordem_cmp']    = "";
   foreach ($campos_sel as $campo_sort)
   {
       ...
       if (isset($tab_def_campos[$campo]))
       {
           ...
           if (isset($_SESSION['sc_session'][$sc_init]['app_grid_sec_apps']['ordem_quebra']))
           {
               foreach($_SESSION['sc_session'][$sc_init]['app_grid_sec_apps']['ordem_quebra'] as $campo_quebra => $resto) 
               {
                   foreach($resto as $sqldef => $ordem_ant) 
                   {
                       if ($sqldef == $tab_def_campos[$campo]) 
                       { 
                           $_SESSION['sc_session'][$sc_init]['app_grid_sec_apps']['ordem_quebra'][$campo][$sqldef] = $ordem;
           ...
           if (!$Ordem_tem_quebra)
           {
               $_SESSION['sc_session'][$sc_init]['app_grid_sec_apps']['ordem_select'][$tab_def_campos[$campo]] = $ordem;
           }   
       }
   }
?>
...
?>
  function scSubmitOrderCampos(sPos, sType) {
   ...
   return new Promise(function(resolve, reject) {$.ajax({
    type: "POST",
    url: "app_grid_sec_apps_order_campos.php",
    data: {
...
<?php
   ...
   $arr_order = ($this->restore) ? $_SESSION['sc_session'][$sc_init]['app_grid_sec_apps']['ordem_select_orig'] : $_SESSION['sc_session'][$sc_init]['app_grid_sec_apps']['ordem_select'];
...
<?php
  if (isset($_SESSION['sc_session'][$sc_init]['app_grid_sec_apps']['ordem_select']) && $_SESSION['sc_session'][$sc_init]['app_grid_sec_apps']['ordem_select'] != $_SESSION['sc_session'][$sc_init]['app_grid_sec_apps']['ordem_select_orig']) {
      $disp_rest = "";
  }
   if (!$embbed)
   {
