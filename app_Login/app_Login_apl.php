...
$_SESSION['scriptcase']['app_Login']['contr_erro'] = 'off'; 
    ...
    function ValidateField_login(&$Campos_Crit, &$Campos_Falta, &$Campos_Erros)
    {
      ...
      { 
          ...
          { 
              $hasError = true;
              $Campos_Crit .= "Usuario " . $this->Ini->Nm_lang['lang_errm_mnch'] . " 3 " . $this->Ini->Nm_lang['lang_errm_nchr']; 
              if (!isset($Campos_Erros['login']))
              {
                  $Campos_Erros['login'] = array();
              }
    ...
    function ValidateField_pswd(&$Campos_Crit, &$Campos_Falta, &$Campos_Erros)
    {
      ...
      { 
          ...
          { 
              $hasError = true;
              $Campos_Crit .= "Contraseña " . $this->Ini->Nm_lang['lang_errm_mxch'] . " 32 " . $this->Ini->Nm_lang['lang_errm_nchr']; 
              if (!isset($Campos_Erros['pswd']))
              {
                  $Campos_Erros['pswd'] = array();
              }
          ...
          { 
              $hasError = true;
              $Campos_Crit .= "Contraseña " . $this->Ini->Nm_lang['lang_errm_mnch'] . " 3 " . $this->Ini->Nm_lang['lang_errm_nchr']; 
              if (!isset($Campos_Erros['pswd']))
              {
                  $Campos_Erros['pswd'] = array();
              }
...
;
        ...
        if (is_file($_SESSION['scriptcase']['dir_temp'] . "/sc_apl_default_coprogreso.txt")) {
    unlink($_SESSION['scriptcase']['dir_temp'] . "/sc_apl_default_coprogreso.txt");
}
if (is_file("../_lib/friendly_url/app_Login_ini.txt")) {
    $SC_arq_def = fopen($_SESSION['scriptcase']['dir_temp'] . "/sc_apl_default_coprogreso.txt", "w");
    fwrite ($SC_arq_def, 'app_Login, R');
    fclose ($SC_arq_def);
    setcookie('sc_apl_default_coprogreso','app_Login, R','0','/');
}
;
		 if (isset($this->sc_temp_usr_login)) { $_SESSION['usr_login'] = $this->sc_temp_usr_login;}
 if (!isset($this->Campos_Mens_erro) || empty($this->Campos_Mens_erro))
